...
def write_imf_params_table():
    ...
    """
    table_rows = []  # (run, label, m_c samples, Gamma samples, m_max samples, sfe_arr)
    for run in IMF_RUNS:
        samples_path = run.path + f"/imf_samples_jax/samples_{model}.npy"
        ...
        m_max  = 10 ** samples[:, 4]
        sfe    = _sfe_values(run)
        table_rows.append((run, _table_label(run), m_peak, gamma, m_max, sfe))

    def _fmt_mcld(m):
        """Format cloud mass with a compact power-of-10: e.g. $2\times10^4$."""
        exp = int(np.floor(np.log10(m)))
        coeff = m / 10**exp
        if abs(coeff - round(coeff)) < 0.05:
            return rf"${round(coeff):.0f}\times10^{{{exp}}}$"
    ...
    # ---- LaTeX ----
    tex_rows = []
    for run, lbl, mp, g, mm, sfe in table_rows:
        z_str = f"${10.0**run.logZ:g}$"
        tex_rows.append(
            f"{lbl} & {_fmt_mcld(run.M_cloud)} & ${run.R_cloud:g}$ & {z_str} & "
            f"{_fmt_sfe(sfe)} & {_fmt_uncertain(mp)} & {_fmt_uncertain(g)} & {_fmt_uncertain(mm)} \\\\"
        )
    tex = (
        ...
        "\\toprule\n"
        "Simulation & $M_{\\rm cloud}\\,(M_\\odot)$ & $R_{\\rm cloud}\\,(\\mathrm{pc})$ & "
        "$Z/Z_\\odot$ & SFE & $m_c\\,(M_\\odot)$ & $\\Gamma$ & "
        "$M_{\\max}\\,(M_\\odot)$ \\\\\n"
        "\\midrule\n"
        + "\n".join(tex_rows) + "\n"
        "\\bottomrule\n"
        ...
        "Salpeter $=-1.35$), $M_{\\max}$ is the upper-mass cutoff, and "
    ...
    header = (
        f"{'Simulation':<{col1w}}  {'M_cloud':>10}  {'R_cloud':>8}  {'Z/Zsun':>6}"
        f"  {'SFE':>16}  {'m_c (Msun)':>24}  {'Gamma':>24}  {'M_max (Msun)':>24}"
    )
    sep = "-" * len(header)
    lines = [sep, header, sep]
    for run, lbl, mp, g, mm, sfe in table_rows:
        name = lbl.replace(r"\texttt{", "").replace("}", "").replace(r"\_", "_")
        z_val = f"{10.0**run.logZ:g}"
        lines.append(
            f"{name:<{col1w}}  {run.M_cloud:>10.3g}  {run.R_cloud:>8.3g}  {z_val:>6}"
            f"  {_ascii_sfe(sfe):>16}  {_ascii_val(mp):>24}  {_ascii_val(g):>24}  {_ascii_val(mm):>24}"
        )
    lines.append(sep)
    ascii_text = "\n".join(lines) + "\n"
