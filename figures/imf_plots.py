...
def write_imf_params_table():
    ...
    int_tex = (
        ...
        "\\bottomrule\n"
        "\\end{tabular}\n"
        "\\caption{Integrated properties of the simulated star clusters. "
        "SFE is the final star formation efficiency; "
        "$L/M_*$ is the specific bolometric luminosity and "
        "$\\dot{Q}_{\\rm H}/M_*$ the specific ionizing photon rate, "
        "both computed directly from the stellar catalog at the end of the simulation. "
        "For runs with multiple turbulent realizations, SFE is quoted as mean\\,$\\pm$\\,std. "
        "Entries marked \\ldots\\ have no stellar catalog in the local data directory.}\n"
    ...
    imf_tex = (
        ...
        "fit to each simulated stellar sample. "
        "Errors on $m_c$, $\\sigma$, $\\Gamma$, and $M_{\\max}$ are 16th/84th posterior percentiles. "
        "$m_c$ is the lognormal characteristic mass, "
        "$\\sigma$ is the lognormal width in dex, "
        "$\\Gamma$ is the high-mass slope ($\\mathrm{d}N/\\mathrm{d}\\log m \\propto m^\\Gamma$; "
        "Salpeter $=-1.35$), and $M_{\\max}$ is the upper-mass cutoff. "
        "Entries marked \\ldots\\ have no stellar catalog in the local data directory. "
        "The bottom two rows give canonical published values from the literature (no uncertainties or $M_{\\max}$ are defined).}\n"
